...
const SelectedProject = ({
  ...
  const getCurrentProjects = async () => {
    ...
    let url = `${currentUserDoc.companyName}/${bankData[0].id}/Loan Officers/${currentUser.user.uid}/LOusers/${selectedArchivedData.userId}/Project Information/${selectedArchivedData.projectName}/Milestone`
    ...
    let usedMileStonePercentage = ((totalMileStoneValue / selectedArchivedData.totalloanamount) * 100).toFixed(5);
    ...
    <section className="selectedProject"  >
      ...
      <div className="selectedProject-header header">
        <h1 style={{ margin: "2.2rem 0 0.5rem 0" }}>
          {selectedArchivedData.projectName}
        </h1>
        <hr
          style={{
            maxWidth: "14rem",
      ...
      <div className="selectedProject-details" id="report">
        <div className="selectedProject-details-loan">
          <h1>Total Loan</h1>
          <h1>${selectedArchivedData.totalloanamount}</h1>
        </div>
